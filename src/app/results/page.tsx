        ...
        periods && periods.length >= 1 ?
          <div className="w-full flex justify-center mt-10">
            <Table className="max-w-[1440px] mx-auto">
              <TableHeader>
                <TableRow>
                  <TableHead className="text-yellow-400 text-center">Period</TableHead>
                  <TableHead className="text-yellow-400 text-center">Awards achieved</TableHead>
                  <TableHead className="text-yellow-400 text-center">Total prize earned</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
              {
                periods.map(( period, index ) => (
                  <TableRow key={`period-${index}-${period.id}`} className="h-[68px]">
                    <TableCell className="text-center">{period.name}</TableCell>
                    ...
                    <TableCell className="flex gap-4 items-center justify-center">

                      {
                        period.achieved_1 && awards?.find(award => award.id === period.achieved_1)?.id && awards?.find(award => award.id === period.achieved_1)?.icon ?
                          <Image src={awards?.find(award => award.id === period.achieved_1)?.icon ?? ''} alt="Award" width={48} height={48} />
                          : null
                      }

                      {
                        period.achieved_2 && awards?.find(award => award.id === period.achieved_2)?.id && awards?.find(award => award.id === period.achieved_2)?.icon ?
                          <Image src={awards?.find(award => award.id === period.achieved_2)?.icon ?? ''} alt="Award" width={48} height={48} />
                          : null
                      }

                      {
                        period.achieved_3 && awards?.find(award => award.id === period.achieved_3)?.id && awards?.find(award => award.id === period.achieved_3)?.icon ?
                          <Image src={awards?.find(award => award.id === period.achieved_3)?.icon ?? ''} alt="Award" width={48} height={48} />
                          : null
                      }
