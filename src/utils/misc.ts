...
export const calculateAwardPot = ( daysSinceLastCriticalError: number, awards: IAward[], period: IPeriod ): number => {
  ...
  if ( daysSinceLastCriticalError && period && awardsList && awardsList.length >= 3 ) {
    ...
    if ( daysSinceLastCriticalError >= awardsList[0]!.days_required && period.achieved_1 ) {
      result_month_1 =
    ...
    if ( daysSinceLastCriticalError >= awardsList[1]!.days_required && period.achieved_2 ) {
      result_month_2 =
    ...
    if ( daysSinceLastCriticalError >= awardsList[2]!.days_required && period.achieved_3 ) {
      result_month_3 =
        // Health of the 2nd month of the period determines the award value
